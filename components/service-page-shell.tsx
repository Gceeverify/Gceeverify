...
export function ServicePageShell({ eyebrow, title, description, children }: { eyebrow: string; title: string; description: string; children: ReactNode }) {
  return <main className="service-marketplace min-h-screen text-white">
    ...
    <header className="market-topbar"><Link href="/#dashboard" className="brand-logo" aria-label="GCVerify dashboard"><Image src="/gcverify-logo-light.png" width={4138} height={928} alt="GCVerify" priority /></Link><div className="flex items-center gap-2"><button className="icon-button" aria-label="Support"><Headphones /></button><button className="icon-button" aria-label="Notifications"><Bell /></button><button className="market-balance"><WalletCards /><span>Wallet</span><strong>$128.40</strong></button></div></header>
