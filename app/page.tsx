...
import {
  ArrowUpRight, Bell, Check, ChevronRight, CircleHelp, Headphones,
  LayoutDashboard, ListChecks, Menu, PackageSearch, Plus, Search,
  ShoppingBag, Server, Smartphone, TicketCheck, WalletCards, X, Zap,
} from 'lucide-react';
      ...
      <section className="md:pl-[250px]">
        ...
        <div className="dashboard-content mx-auto max-w-[1420px] px-4 py-7 sm:px-7 lg:px-10 lg:py-9">
          <div className="dashboard-welcome">
            <div><p className="mb-2 flex items-center gap-2 text-sm text-white/40"><span className="size-2 rounded-full bg-lime-300 shadow-[0_0_10px_#bef264]" />All systems operational</p><h1>Good morning, Ola.</h1></div>
            <p>Manage your digital services from one place.</p>
          </div>

          <section className="dashboard-balance" aria-label="Wallet balance">
            <div><span>Available balance</span><strong>$128.40</strong></div>
            <Button className="dashboard-balance-button"><Plus /> Add funds</Button>
          </section>

          <section id="dashboard" className="dashboard-services">
            <div className="dashboard-section-heading"><p className="eyebrow">Services</p><h2>What do you need?</h2></div>
            <div className="dashboard-service-shortcuts">{quickActions.map(({ href, label, icon: Icon, tone }) => <Link href={href} key={href} className="dashboard-service-shortcut"><span className={`dashboard-shortcut-icon shortcut-${tone}`}><Icon /></span><strong>{label}</strong></Link>)}</div>
          </section>

          <section className="panel recent-activity">
            <div className="recent-activity-head"><div><p className="eyebrow">Activity</p><h2>Recent orders</h2></div><button className="text-sm font-medium text-lime-300">View all</button></div>
            <div className="recent-order-list">{recentOrders.map((order) => <article key={order.id} className="recent-order-row"><div className="recent-order-main"><span className="recent-order-icon"><PackageSearch /></span><div><strong>{order.service}</strong><small>{order.id} · {order.qty}</small></div></div><div className="recent-order-meta"><span className={`status ${order.status === 'Completed' ? 'status-complete' : 'status-progress'}`}><span />{order.status}</span><small>{order.time}</small></div></article>)}</div>
          </section>
        </div>
      </section>
    </main>
